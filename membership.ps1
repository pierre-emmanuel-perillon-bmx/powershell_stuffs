$INI_GROUPAD = 'CORP-G-APP-W11-VDI-Test-User'
$INI_ROLENAME = 'WINDOWS_11_VIRTUAL_COMPUTER_Test_User'
$INI_BIOACCESS_EXTRACT  ='./user_roles.csv'
$INI_BIOACCESS_CACHE  = './user_cached.csv'
...
function search-habilitation {
    ...
    process {
        foreach ($x in $users) { 
            ...
            if ( $bioaccess.ContainsKey($id) ){
                $biorole  = ' with bioaccess role '
            }
            else {
                $biorole =' not in bioaccess '
            }
...
update-FileToCache $INI_BIOACCESS_EXTRACT $INI_BIOACCESS_CACHE $INI_ROLENAME
$rolesHtb = read-Cache  $INI_BIOACCESS_CACHE
